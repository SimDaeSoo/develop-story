...
export async function _gerServerConfig() {
    try {
        const response = await axios.get(`${process.env.BASE_API_URL}/config`);
...
export async function _verifing(provider, access_token, id_token) {
    try {
        const response = await axios.get(`${process.env.BASE_API_URL}/auth/${provider}/callback?${stringify({ access_token, id_token })}`);
